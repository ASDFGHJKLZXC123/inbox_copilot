function ReminderThreadPanel({ selectedThreadId, reminders, onRemindersChange, onError, open, setOpen }) {
  ...
  useEffect(() => {
    setDueAt(''); setReason(''); setPending(false);
  }, [selectedThreadId]);
  ...
  const toggleComplete = (id, completed) => {
    onRemindersChange(reminders.map((r) => r.id === id ? { ...r, completed } : r));
  };
  const remove = (id) => {
    ...
    <div className="relative">
      <button
        ...
        className={
          ...
          (threadReminders.some((r) => !r.completed)
      ...
      >
        <I.Bell size={12} />
        <span>Follow-up</span>
        {threadReminders.filter((r) => !r.completed).length > 0 && (
          <span className="tabular-nums">{threadReminders.filter((r) => !r.completed).length}</span>
        )}
      </button>
        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-1.5 z-40 w-[360px] bg-slate-900 border border-slate-800 rounded-lg shadow-2xl shadow-black/40 overflow-hidden">
            <div className="px-3.5 py-2.5 border-b border-slate-800 flex items-center gap-2">
              ...
              <span className="text-[12px] font-semibold text-slate-100">Follow-ups on this thread</span>
            </div>

            {/* List */}
            <div className="max-h-60 overflow-y-auto">
              {threadReminders.length === 0 ? (
                <div className="px-4 py-5 text-center text-[12px] text-slate-500">No follow-ups scheduled on this thread.</div>
              ) : (
                <ul className="py-1">
                  {threadReminders.map((r) => (
                    <li key={r.id} className="px-3.5 py-2 flex items-start gap-2.5 hover:bg-slate-800/40">
                      <button
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={'text-[12px] leading-snug ' + (r.completed ? 'text-slate-500 line-through' : 'text-slate-100')}>
                          {r.reason}
                        </div>
                        <div className="text-[10.5px] text-amber-300/80 mt-0.5 flex items-center gap-1">
                          <I.Clock size={9} />
                          {MOCK.fullTimestamp(r.dueAt)}
                        </div>
                      </div>
                      <button
                        onClick={() => remove(r.id)}
            ...
            <div className="px-3.5 py-3 border-t border-slate-800 bg-slate-950/60 space-y-2">
              <div className="text-[10.5px] uppercase tracking-wide text-slate-500 font-medium">Add follow-up</div>
              <input
                type="text"
...
window.ReminderThreadPanel = ReminderThreadPanel;
