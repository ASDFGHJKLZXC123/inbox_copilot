...
function Sidebar({ activeNav, setActiveNav, session, folderCounts, onCompose, onSignOut, onClearCache, onOpenPreferences, accountMenuOpen, setAccountMenuOpen }) {
  return (
    <aside className="w-[228px] flex-shrink-0 border-r border-slate-800/80 bg-slate-950 flex flex-col">
      ...
      <div className="h-14 px-4 flex items-center gap-2.5 border-b border-slate-800/80">
        ...
        <button onClick={onOpenPreferences} className="text-slate-500 hover:text-slate-200 transition-colors" title="Preferences">
          <I.Settings size={14} />
        </button>
      </div>
      ...
      <nav className="px-2 pt-4 flex-1">
        <div className="px-2 pb-1.5 text-[10.5px] uppercase tracking-[0.08em] text-slate-500 font-medium">Mailboxes</div>
        <ul className="space-y-0.5">
          {MOCK.NAV_ITEMS.map((item) => {
            const active = activeNav === item.id;
            const Icon = { inbox: I.Inbox, sent: I.Send, drafts: I.Draft, archive: I.Archive, trash: I.Trash }[item.id];
        ...
        <div className="mt-6 px-2 pb-1.5 text-[10.5px] uppercase tracking-[0.08em] text-slate-500 font-medium">Copilot</div>
        <ul className="space-y-0.5">
          <li>
            <button className="group w-full h-8 px-2 rounded-md flex items-center gap-2.5 text-[13px] text-slate-400 hover:bg-slate-800/40 hover:text-slate-200 transition-colors">
              ...
              <span className="text-[11px] font-medium text-amber-300/90 tabular-nums">2</span>
            </button>
          </li>
          <li>
            <button className="group w-full h-8 px-2 rounded-md flex items-center gap-2.5 text-[13px] text-slate-400 hover:bg-slate-800/40 hover:text-slate-200 transition-colors">
      ...
      <div className="relative border-t border-slate-800/80">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12.5px] text-slate-100 font-medium truncate leading-tight">{session.user.name}</div>
            <div className="text-[11px] text-slate-500 truncate">{session.user.email}</div>
          </div>
          <I.ChevronUp size={13} className={'text-slate-500 transition-transform ' + (accountMenuOpen ? '' : 'rotate-180')} />
        </button>
      </div>
    </aside>
  );
...
function AccountMenu({ session, onSignOut, onClearCache, onOpenPreferences, onClose }) {
  return (
    <>
      ...
      <div className="absolute bottom-full left-3 right-3 mb-2 z-30 bg-slate-900 border border-slate-800 rounded-lg shadow-2xl shadow-black/40 overflow-hidden">
        ...
        <div className="py-1">
          <MenuItem icon={I.Settings} label="Preferences" onClick={() => { onClose(); onOpenPreferences && onOpenPreferences(); }} />
          <MenuItem icon={I.Refresh} label="Resync this account" onClick={onClose} />
        </div>
        <div className="py-1 border-t border-slate-800">
          <MenuItem icon={I.Trash} label="Clear local cache…" destructive onClick={() => { onClose(); onClearCache(); }} />
          <MenuItem icon={I.LogOut} label="Sign out" onClick={() => { onClose(); onSignOut(); }} />
        </div>
      </div>
    </>
  );
