...
import type { ModifyAction, ThreadCard } from "@/lib/types-ui";
...
function bannerFor(authError: string | undefined): AuthBanner | null {
  if (!authError) return null;
  if (authError === "RefreshAccessTokenError") {
    return {
      message: "Your Google connection expired. Reconnect to keep syncing.",
      cta: "Reconnect",
      action: () => void signIn("google"),
    };
  }
  if (authError === "MissingRefreshToken") {
    return {
      message: "Sign in again to enable AI features.",
      cta: "Reconnect",
      action: () => void signIn("google"),
    };
  }
  return {
    message: "Auth error — sign out and back in.",
    cta: "Sign out",
    action: () => {
...
export function EmailDetailPanel({
  ...
  const [expandedIds, setExpandedIds] = useState<Set<string>>(() =>
    threadMessages.length
      ? new Set([threadMessages[threadMessages.length - 1].id])
      : new Set<string>(),
  );
  const [sending, setSending] = useState(false);
  const banner = bannerFor(authError);
  ...
  const replyTo = lastMessage ? nameOf(lastMessage.from) : "";
    ...
    <section className="flex-1 min-w-0 flex flex-col bg-slate-950">
      ...
      {banner && (
        <div className="px-4 py-2 bg-amber-400/10 border-b border-amber-400/20 flex items-center gap-2 text-[11.5px] text-amber-200">
          ...
          <button
            ...
            className="ml-auto h-6 px-2.5 rounded bg-amber-300 text-slate-950 font-medium text-[11px]"
          >
            {banner.cta}
          </button>
        </div>
      ...
      <div className="h-14 px-4 flex items-center gap-1.5 border-b border-slate-800/80">
        ...
        <ToolbarButton
          icon={I.Archive}
          label="Archive (E)"
          onClick={() => void onModify("archive")}
          disabled={modifying}
        />
        <ToolbarButton
          icon={I.Trash}
          label="Trash (#)"
          onClick={() => void onModify("trash")}
          disabled={modifying}
        />
        <ToolbarButton
          icon={I.Clock}
          label="Snooze"
          onClick={() => onError("Snooze isn't wired up yet")}
        />
        <ToolbarButton
          icon={I.Bell}
          label="Mark unread"
          onClick={() => void onModify("mark-unread")}
          disabled={modifying}
        />
        ...
        <div className="ml-auto flex items-center gap-1.5">{reminderPanelSlot}</div>
      </div>
