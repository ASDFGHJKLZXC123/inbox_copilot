function EmailList({
  ...
  const folderLabel = MOCK.NAV_ITEMS.find((n) => n.id === activeNav)?.label || 'Inbox';
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="h-14 px-4 flex items-center gap-3 border-b border-slate-800/80">
        ...
        <button className="text-slate-500 hover:text-slate-200" title="More" aria-label="More options">
          <I.More size={14} />
        </button>
      </div>
      ...
      <div className="px-3 pt-3 pb-2.5 border-b border-slate-800/60 space-y-2.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <FilterChip
            ...
            disabled={isSearchMode}
          >
            Unread {totalUnread > 0 && <span className="ml-1 tabular-nums text-slate-500">{totalUnread}</span>}
          </FilterChip>
          {isSearchMode && (
            <span className="ml-auto text-[10.5px] text-slate-500 italic">Filters paused — search active</span>
          )}
        </div>
      </div>
...
function ThreadRow({ card, selected, onSelect, hasReminder }) {
  const unread = card.unreadCount > 0;
  const lastFrom = MOCK.nameOf(card.participants[0]);
  ...
  return (
    <li>
      ...
      >
        ...
        {unread && !selected && <span className="absolute left-1.5 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-sky-400" />}

        <div className="flex items-baseline gap-2 mb-1">
          <span className={'text-[12.5px] truncate flex-1 ' + (unread ? 'text-slate-100 font-semibold' : 'text-slate-300 font-medium')}>
            {lastFrom}{others && <span className="text-slate-500">{others}</span>}
          </span>
          <span className="text-[10.5px] text-slate-500 tabular-nums flex-shrink-0">
            {MOCK.smartTimestamp(card.lastMessageAt)}
          </span>
        </div>
        <div className={'text-[12.5px] truncate mb-0.5 ' + (unread ? 'text-slate-100 font-medium' : 'text-slate-300')}>
          {card.subject}
        </div>
        <div className="text-[11.5px] text-slate-500 line-clamp-1 leading-relaxed">
          {card.preview}
        </div>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center px-8 py-16 text-center">
      ...
      <div className="text-[12px] text-slate-500 max-w-[220px] leading-relaxed">
        {filter === 'unread' ? 'No unread mail in this folder.' : 'Nothing here yet. New mail will appear automatically.'}
      </div>
    </div>
  );
}
