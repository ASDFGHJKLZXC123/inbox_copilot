function ClearCacheDialog({ onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 overlay-in" style={{ background: 'rgba(2, 6, 23, 0.65)', backdropFilter: 'blur(4px)' }}>
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="px-5 py-3 flex items-center gap-2 justify-end border-t border-slate-800 bg-slate-950/40">
          <button
            onClick={onClose}
            className="h-8 px-3 rounded-md text-[12.5px] text-slate-300 hover:bg-slate-800 transition-colors"
          >
            Cancel
          </button>
          <button
...
window.ClearCacheDialog = ClearCacheDialog;
